...
// ── Porcentajes de posición (entre 0 y 1) ───────────────────
// En vez de píxeles fijos, todo se calcula como porcentaje del canvas
// así se adapta a cualquier tamaño de ventana

// Pooh en escenas normales
const POOH_PW = 0.15; // ancho de Pooh = 15% del ancho del canvas
const POOH_PH = 0.38; // alto de Pooh = 38% del alto del canvas
const POOH_PY = 0.62; // Y de Pooh = 62% del alto del canvas

// Pooh fijo en las escenas de la miel (lado izquierdo)
const POOH_MIEL_PX = 0.15; // X = 15% del ancho

// Posición Y de las nubes de texto
const TEXTO_TITULO_PY = 0.08; // texto principal, arriba
const TEXTO_HINT_PY = 0.17; // texto secundario (instrucción)

// Área clickeable de la miel (toda la pantalla)
const MIEL_PX = 0;
const MIEL_PY = 0;
const MIEL_PW = 1.0;
const MIEL_PH = 1.0;

// ── Helpers: convierten porcentaje a píxeles reales según el canvas actual ─
const pw = (p) => width * p; // porcentaje del ancho
const ph = (p) => height * p; // porcentaje del alto

// ============================================================
// VARIABLES INTERNAS - estas cambian durante el codigo
// ============================================================
...
function setup() {
  ...
  reiniciar(); // reinicia el codigo
}

// Se llama automáticamente si el usuario redimensiona la ventana
...
function reiniciar() {
  ...
  pooh_x = -pw(POOH_PW); // Pooh inicia fuera de pantalla - lado izquierdo
  frame_caminata = 0;
  ultimo_frame = millis();
...
function draw() {
  ...
  // para entenderlo mejor: es como un semáforo - si el estado es INICIO muestra la pantalla de inicio,
  // si es NOS_JUIMOS entonces muestra a Pooh caminando, despues lo mismo con todos los estados,
  // el `switch` revisa en que estado este el juego/codigo y llama a la función que dibujará esa pantalla
  switch (estado) {
    case E.INICIO:
      dibujar_inicio();
      break;
...
function dibujar_inicio() {
  ...
  nube_texto("Presiona cualquier tecla para comenzar", width / 2, ph(0.59), 14);
}
...
function dibujar_dialogo_hambre() {
  ...
  image(pooh, width / 2 - pw(POOH_PW) / 2, ph(POOH_PY), pw(POOH_PW), ph(POOH_PH));
  nube_texto("¡Tengo hambre! Voy a buscar miel…", width / 2, ph(TEXTO_TITULO_PY));
  nube_texto("Presiona una tecla para partir", width / 2, ph(TEXTO_HINT_PY), 14);
}
...
function dibujar_caminando() {
  ...
  image(img_actual || pooh, pooh_x, ph(POOH_PY), pw(POOH_PW), ph(POOH_PH));

  nube_texto("Buscando miel por el bosque…", width / 2, ph(TEXTO_TITULO_PY));
  nube_texto("Presiona una tecla al llegar", width / 2, ph(TEXTO_HINT_PY), 14);
}
...
function dibujar_llego_miel() {
  ...
  image(img_poohladito, pw(POOH_MIEL_PX), ph(POOH_PY), pw(POOH_PW), ph(POOH_PH));
  ...
  if (DEBUG_MIEL) {
    ...
    stroke(255, 0, 0);
    strokeWeight(2);
    rect(pw(MIEL_PX), ph(MIEL_PY), pw(MIEL_PW), ph(MIEL_PH));
    noStroke();
  }

  nube_texto("¡Haz click en la miel!", width / 2, ph(TEXTO_TITULO_PY));
}
...
function dibujar_decision() {
  ...
  image(img_poohagarro, pw(POOH_MIEL_PX), ph(POOH_PY), pw(POOH_PW), ph(POOH_PH));
  nube_texto("¡Encontré la miel, comeré jeje!", width / 2, ph(TEXTO_TITULO_PY));
  nube_texto("(scroll para comer)", width / 2, ph(TEXTO_HINT_PY), 14);
}
...
function dibujar_comiendo() {
  // el fondo usa img_miel si existe, sino se va al fondo de día
  if (img_miel) {
  ...
  image(img_poohcomio, pw(POOH_MIEL_PX), ph(POOH_PY), pw(POOH_PW), ph(POOH_PH));

  nube_texto("¡Qué rica miel! jeje", width / 2, ph(TEXTO_TITULO_PY));
  nube_texto("(scroll para volver a empezar)", width / 2, ph(TEXTO_HINT_PY), 14);
}
...
function nube_texto(msg, x, y, tam = 20) {
  ...
  // recuadro con las esquinas redondeadas
...
function keyPressed() {
  if (estado === E.INICIO) {
    ...
    pooh_x = -pw(POOH_PW); // Pooh parte fuera de pantalla
...
function mousePressed() {
  if (estado === E.LLEGO_LA_MIEL_CASERITO) {
    if (
      mouseX > pw(MIEL_PX) &&
      mouseX < pw(MIEL_PX) + pw(MIEL_PW) &&
      mouseY > ph(MIEL_PY) &&
      mouseY < ph(MIEL_PY) + ph(MIEL_PH)
    ) {
      estado = E.TA_SERVIO; // pasa después de tocar la 'miel' - la pantalla en cualquier lugar
    }
  }
}
...
// ============================================================
// EVENTO DE SCROLL
// Scroll en TA_SERVIO - se pasa a QUEDE_POCHITO.
// Scroll en QUEDE_POCHITO - se reinicia el juego.
